	...
	*/
	public function __construct(
		...
		public function getProfileInformation( $buildSubject ) {
			...
			// Build data
			$body = $this->getProfileRequestBody( $buildSubject );
			...
			if ( (int)$response['status'] === 200 ) {
				if ( !empty( $reponse['profile']['profileNumber'] ) ) {
					$profile_token = $reponse['profile']['profileNumber'];
				}
			}
		...
		public function callPaperlessAPI( array $request_body ) {
			...
			$client->setRawData($request_body, 'application/json');
		 ...
		 * @return array $body
		 */
		public function getProfileRequestBody( array $buildSubject ) {
